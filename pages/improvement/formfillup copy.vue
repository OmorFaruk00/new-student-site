  ...
  methods: {
    ...
    async downloadAdnitcard() {
      ...
      if (!user) {
        ...
        return await this.$axios.get(`/student-improvement-admit-card/${student_id}/${this.currentExamSchedule}/${type}?token=${token}`, {
          responseType: 'blob',
        })
          .then((response) => {
            ...
            download(response.data, `${type}_improvement_admit_card`, content);
